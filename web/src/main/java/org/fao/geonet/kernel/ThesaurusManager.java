  ...
  final class InitThesauriTableTask implements Runnable {
	...
	private void loadRepositories(File thesauriDirectory, String root) {
        ...
        for (String aRdfDataFile : rdfDataFile) {
            	...
            	gst = new Thesaurus(aRdfDataFile, root, thesauriDirectory.getName(), outputRdf, dm.getServiceUrl());

						} else {
            	gst = new Thesaurus(aRdfDataFile, root, thesauriDirectory.getName(), new File(thesauriDirectory, aRdfDataFile), dm.getServiceUrl());
						}
	...
	private void getRegisterMetadataAsRdf(String uuid, OutputStream os) throws Exception {
		...
		try {
			...
			Element env = Lib.prepareTransformEnv(mdInfo.uuid, mdInfo.changeDate, "", dm.getServiceUrl(), "");
	...
	public String createUpdateThesaurusFromRegister(String uuid, String type) throws Exception {
	  ...
	  // check whether we have created a thesaurus for this register already
		...
		Thesaurus gst = new Thesaurus(aRdfDataFile, root, type, outputRdf, dm.getServiceUrl());
