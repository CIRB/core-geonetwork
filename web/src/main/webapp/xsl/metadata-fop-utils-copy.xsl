...
<xsl:stylesheet version="1.0"
				...
				<xsl:call-template name="getTitle">
					<xsl:with-param name="name">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:choose>
									<xsl:when test="name(..)='gmd:linkage'">
										<xsl:value-of select="name(../..)" />
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="name(..)" />
	...
	<xsl:template mode="simpleElementFop" match="@*">
		<xsl:param name="schema" />
		<xsl:param name="title">
			<xsl:if test="name(..)!='gmd:MD_KeywordTypeCode'">
				<xsl:call-template name="getTitle">
					<xsl:with-param name="node" select="../.." />
					<xsl:with-param name="name" select="name(../..)" />
					<!-- Usually codelist -->
	...
	<xsl:template mode="simpleElementFop" match="gfc:FC_FeatureType/@*">
 	...
 	<xsl:template mode="elementFop" match="gmd:MD_Keywords|gmd:CI_ResponsibleParty|gmd:RS_Identifier|gmd:language|gmd:DQ_QuantitativeResult|gmd:role|gmd:DQ_Scope/gmd:level|gmd:MD_LegalConstraints/gmd:accessConstraints|gmd:MD_LegalConstraints/gmd:useConstraints|gmd:dateType|gmd:status|gmd:associationType|gmd:spatialRepresentationType|gmd:hierarchyLevel|gmd:MD_Medium/gmd:name|gmd:classification|gmd:maintenanceAndUpdateFrequency|gmd:characterSet|srv:couplingType|gmd:geometricObjectType" >
		<xsl:param name="schema" />
		<xsl:param name="blockHeaders" />
		<xsl:param name="skipTags" />
	...
	<xsl:template mode="simpleElementFop"
		match="geonet:element|geonet:info|geonet:attribute|geonet:schematronerrors|@codeList|*[@codeList]|@xlink:type|gmd:pass[@gco:nilReason]|@gco:nilReason|gco:UnlimitedInteger/@*|@xsi:type|@gco:isoType" />
	<xsl:template mode="simpleElementFop"
		match="gml:TimePeriod/@gml:id|gml:TimePeriod/@frame|gml:TimePeriod/*/@frame" />
	<xsl:template mode="complexElementFop"
		match="geonet:element|geonet:info|geonet:attribute|geonet:schematronerrors" />

	<xsl:template mode="elementFop" match="gmd:onLine">
		<xsl:param name="schema" />
		<xsl:apply-templates mode="elementFop" select="./gmd:CI_OnlineResource/gmd:linkage">
			<xsl:with-param name="schema" select="$schema" />
		</xsl:apply-templates>

		<xsl:apply-templates mode="elementFop" select="./gmd:CI_OnlineResource/gmd:protocol">
			<xsl:with-param name="schema" select="$schema" />
		</xsl:apply-templates>

		<xsl:apply-templates mode="elementFop" select="./gmd:CI_OnlineResource/gmd:name">
			<xsl:with-param name="schema" select="$schema" />
		</xsl:apply-templates>

		<xsl:apply-templates mode="elementFop" select="./gmd:CI_OnlineResource/gmd:description">
			<xsl:with-param name="schema" select="$schema" />
		</xsl:apply-templates>
	</xsl:template>
</xsl:stylesheet>
