...
<!-- 			<xsl:apply-templates mode="elementFop"
 ...
 -->			<!-- Code -->
			...
			<xsl:for-each select="./gmd:identificationInfo/*/gmd:citation/gmd:CI_Citation/gmd:identifier">
				<xsl:apply-templates mode="elementFop"
					select="./gmd:MD_Identifier/gmd:code|./gmd:RS_Identifier/gmd:code">
					<xsl:with-param name="schema" select="$schema" />
				</xsl:apply-templates>
				<xsl:apply-templates mode="elementFop"
					select="./gmd:MD_Identifier/gmd:codeSpace|./gmd:RS_Identifier/gmd:codeSpace">
					<xsl:with-param name="schema" select="$schema" />
				</xsl:apply-templates>
			</xsl:for-each>
<!-- 
...
-->
			...
			<xsl:for-each select="./gmd:identificationInfo/*/gmd:resourceMaintenance">
						<xsl:apply-templates mode="elementFop" select="./gmd:MD_MaintenanceInformation/gmd:maintenanceAndUpdateFrequency">
							<xsl:with-param name="schema" select="$schema" />
						</xsl:apply-templates>
			</xsl:for-each>
...
-->
					...
					<xsl:for-each select="./gmd:identificationInfo/*/srv:serviceTypeVersion">
		 			...
		 			<xsl:apply-templates mode="elementFop"
						select="./gmd:identificationInfo/*/srv:accessProperties/gmd:MD_StandardOrderProcess">
						<xsl:with-param name="schema" select="$schema" />
					</xsl:apply-templates>
<!--
					</xsl:with-param>
				...
				<xsl:call-template name="newBlock">
					...
					<xsl:with-param name="content">
						<xsl:call-template name="info-blocks">
							<xsl:with-param name="label">
								<xsl:variable name="guiLanguage" select="/root/gui/language" />
								<xsl:choose>
									<xsl:when test="$guiLanguage='dut'">Datasetnaam waarop de service opereert</xsl:when>
									<xsl:when test="$guiLanguage='eng'">Dataset on which the service operates</xsl:when>
									<xsl:otherwise>Nom du jeu de données sur lequel le service opère</xsl:otherwise>
								</xsl:choose>
							</xsl:with-param>
		...
		<xsl:with-param name="content">
			<xsl:apply-templates mode="blockedFop"
				...
				<xsl:with-param name="blockHeaders">
					gmd:distributionFormat|gmd:distributor|gmd:distributionOrderProcess|gmd:transferOptions|gmd:onLine
				</xsl:with-param>
<!-- 			
...
-->
	...
	<xsl:for-each select="./gmd:spatialRepresentationInfo">
		<xsl:call-template name="newBlock">
			...
			<xsl:with-param name="content">
				<xsl:apply-templates mode="elementFop" select="./gmd:MD_VectorSpatialRepresentation/gmd:geometricObjects/gmd:MD_GeometricObjects/gmd:geometricObjectType">
					<xsl:with-param name="schema" select="$schema" />
				</xsl:apply-templates>
			</xsl:with-param>
		</xsl:call-template>
...
<!--
			<xsl:with-param name="blockHeaders">
				...
				gmd:DQ_DomainConsistency|gmd:DQ_ConformanceResult|gmd:specification				
			</xsl:with-param>
-->
			<xsl:with-param name="blockHeaders">
				gmd:dataQualityInfo				
...
-->
	...
	<xsl:template mode="elementFop-iso19139" match="gmd:CI_ResponsibleParty/gmd:role|gmd:DQ_Scope/gmd:level|gmd:MD_LegalConstraints/gmd:accessConstraints|gmd:MD_LegalConstraints/gmd:useConstraints|gmd:dateType|gmd:status|gmd:associationType|gmd:spatialRepresentationType|gmd:hierarchyLevel|gmd:MD_Medium/gmd:name|gmd:classification|gmd:MD_MaintenanceInformation/gmd:maintenanceAndUpdateFrequency|gmd:characterSet|srv:couplingType|gmd:geometricObjectType" >
		<xsl:param name="schema" />
...
<!--
        ...
        <xsl:apply-templates mode="simpleElementFop" select=".">
            ...
            <xsl:with-param name="text">
				...
				<xsl:message select="$codeDescription"/>
...
-->
		...
		<xsl:call-template name="info-blocks">
			...
			<xsl:with-param name="value">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="/root/gui/schemas/*[name(.)=$schema or name(.)='iso19139']/strings/*[name(.)=$bool]"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:with-param>
		</xsl:call-template>
	...
	<xsl:template mode="elementFop-iso19139" match="gmd:language">
		...
		<xsl:call-template name="info-blocks">
			...
			<xsl:with-param name="value">
				<xsl:variable name="guiLanguage" select="/root/gui/language" />
				<xsl:value-of select="/root/gui/isolanguages/record[code=$lang]/label/child::*[name() = $guiLanguage]"/>
				<xsl:if test="../gmd:locale or ../../gmd:locale">
					...
					<xsl:for-each select="../gmd:locale|../../gmd:locale">
		            	...
		            	<xsl:value-of select="/root/gui/isoLang/record[code=$c]/label/child::*[name() = $guiLanguage]"/>
